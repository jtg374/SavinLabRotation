...
def Lengyel2005FullTest(nIter=10,randSeed=(None,None,None)):
    ...
    def odesolve(t_span,x_0):
        ...
        return {'t':t,'y':x,'t_events':x_fire,'final':xNow}
    #%% main
    ...
    for mainIter in range(nIter):
        ...
        for k in range(M):      # the kth memory trace is
            ...
            solFull = odesolve(t_span=(0,param.tf),x_0=x_0)
            solutions += [solFull]
            results[mainIter][:,k] = solFull['final']
    return memorys,cues,synapses,results,solutions
#%%
memorys,cues,synapses,results,solutions = Lengyel2005FullTest(3)
np.save('xMemoryList_1230',memorys)
np.save('xCueList_1230',cues)
np.save('WList_1230',synapses)
np.save('xFinalList_1230',results)
import pickle
with open('firing_1230.data','wb') as f:
